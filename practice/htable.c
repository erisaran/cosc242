...
int htable_insert(htable h, char *str){
    ...
    if (h->keys[index] == NULL){
        ...
        */
        init = container_new(FLEX_ARRAY);
        container_add(keys[index], str);
        h->freq[index] = 1;
        h->num_keys ++;
        return 1;
        ...
        if (h->num_keys == h->capacity){
            ...
            for (i = 1; i< h->capacity; i++){
                if (h->keys[(index + i)%c] == NULL){
                    h->keys[(index +i)% c] = emalloc((strlen(str)+1) * sizeof h->keys[0][0]);
                    strcpy(h->keys[(index +i)%c],str);
                    h->freq[(index + i)%c] = 1;
                    h->num_keys ++;
                    return 1;
                }else if (strcmp(h->keys[(index + i)%c],str) == 0){
                    h->freq[(index + i)%c] ++;
                    return h->freq[(index + i)%c];
                }
